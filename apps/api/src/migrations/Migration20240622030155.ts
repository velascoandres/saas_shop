...
export class Migration20240622030155 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "role" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(255) not null);',
    )
    this.addSql(
    ...
    this.addSql(
      'create table "user" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "first_name" varchar(255) not null, "last_name" varchar(255) not null, "email" varchar(255) not null, "is_active" boolean not null, constraint "user_pkey" primary key ("id"));',
    )
    this.addSql('create index "user_is_active_index" on "user" ("is_active");')

    this.addSql(
      'create table "user_role" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "user_id" uuid not null, "role_id" int not null);',
    )
    this.addSql(
